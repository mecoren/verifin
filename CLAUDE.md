...
- 转账（`EntryType.transfer`）同时影响转出/转入账户余额，但不计入收支统计（聚合逻辑在 [lib/app/ledger_math.dart](lib/app/ledger_math.dart)）。转账可带手续费 `LedgerEntry.fee`，由转出账户承担（转出扣「金额+手续费」、转入只加金额）。
